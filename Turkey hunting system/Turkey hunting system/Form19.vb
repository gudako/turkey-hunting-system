Public Class Form19
    Private LockedLife1 As String, LockedLife2 As String, OriginalAtropos As Point, CardList(1) As ListBox, Handcards(4) As PictureBox, Enable As Boolean = True
    Private TimeLabel As New Label With {.Font = New Font("等线", 48.0!, FontStyle.Regular, GraphicsUnit.Point, 134), .ForeColor = Color.White, .BackColor = Color.Transparent, .Height = 150, .Width = 200, .TextAlign = ContentAlignment.MiddleCenter}
    Private MovingCard As New PictureBox, MovingCardEx As New PictureBox, MoveTarget As PictureBox, HandCardList(1) As ListBox, OneSuccess As Boolean = False
    Private TotalLong As Integer, TotalLongEx As Integer, PlayedCard(1) As String, Statuses(1, 2) As PictureBox, damage(1) As PictureBox, point(1) As String, originalDamageSize As Size, DamageDelay1 As Integer, DamageDelay2 As Integer, ThrowCardOpposite As Boolean
    ''' <summary>
    ''' 窗体必要的初始化。
    ''' </summary>
    Public Sub Initializate()
        ...
        tutorial.Text = "Here is the battlefield of the final battle between you and Atropos. Actually, it's a card game based on turns, each player can only play one card per turn."
        tutorial_next.Tag = 0
    ...
    Private Sub ModifyLife(ByVal player1 As Boolean, value As Single)
        If player1 Then
            ...
            frontbar1.BackColor = Color.FromArgb(255 * (1 - value / UnlockNumber(backbar1.Tag)), 255 * (value / UnlockNumber(backbar1.Tag)), 0)
        Else
            ...
            frontbar2.BackColor = Color.FromArgb(255 * (1 - value / UnlockNumber(backbar2.Tag)), 255 * (value / UnlockNumber(backbar2.Tag)), 0)
        End If
        If UnlockNumber(LockedLife1) = 0 Or UnlockNumber(LockedLife2) = 0 Then
            ...
            ElseIf frontbar2.Visible And UnlockNumber(lockedlife2) = 0 Then
                ...
                If Not Form1.mute.Checked Then My.Computer.Audio.Play(My.Computer.FileSystem.SpecialDirectories.MyDocuments & "\temporary files\sound1.wm")
            End If
        End If
    End Sub
    ''' <summary>
    ...
    ''' <returns></returns>
    Private Function LockNumber(ByVal number As Single) As String
    ...
    Private Sub TimeCost_Tick(sender As Object, e As EventArgs) Handles TimeCost.Tick
        ...
        If TimeLabel.Text = 0 Then
            If HasStatus(False, "timed") Then
                ...
                My.Computer.Audio.Play(My.Computer.FileSystem.SpecialDirectories.MyDocuments & "\temporary files\sound62.wm")
            End If
            PlayCard(card0, 0)
        End If
    End Sub
    ...
    Private Sub tutorial_next_Click(sender As Object, e As EventArgs) Handles tutorial_next.Click
        If tutorial_next.Tag = 0 Then
            tutorial_next.Tag = 1
            tutorial.Text = "The cards contain character cards and magic cards, the most cards are character cards. When the two players put out their two character cards to the board, they will have a battle."
        ElseIf tutorial_next.Tag = 1 Then
            tutorial_next.Tag = 2
            tutorial_image1.Visible = True
            tutorial.Text = "Here is the detail Of the restraint relationship." & vbCrLf & """half"" means" & vbCrLf & "0.5 damage." & vbCrLf & "Once finished," & vbCrLf & "the loser of" & vbCrLf & "the combat" & vbCrLf & "will be" & vbCrLf & "correspondin-" & vbCrLf & "gly damaged."
        ...
        ElseIf tutorial_next.Tag = 3 Then
            ...
            tutorial.Text = "The magic card can cause some special effect to yourself or the enemy, the effect will be shown on the card."
        ...
        ElseIf tutorial_next.Tag = 5 Then
            ...
            tutorial.Text = "Here're the special items got in the game, they can give you some new kind of cards being used in the game."
...
Non_card:
            ...
            tutorial.Text = "It's not so easy to get a magic card: the probability is quite low. Besides luck, you should also try to use your cards wisely."
    ...
    Private Sub MoveCardEx_Tick(sender As Object, e As EventArgs) Handles MoveCardEx.Tick
        ...
        Else
            ...
            MovingCardEx.Size = New Size((1 - hypotenuse / TotalLongEx) * (board1.Width - card0.Width) + card0.Width, (1 - hypotenuse / TotalLongEx) * (board1.Height - card0.Height) + card0.Height)
            If MoveTop >= 10 Then
    ...
    Private Sub MoveCard_Tick(sender As Object, e As EventArgs) Handles MoveCard.Tick
        If MoveCard.Tag <> -1 Then
            ...
            Dim PerLeft As Integer = 1 * average
            Dim PerTop As Integer = 10 - PerLeft
            If MoveCard.Tag = 2 Then MovingCard.Size = New Size((1 - hypotenuse / TotalLong) * (board2.Width - card0.Width) + card0.Width, (1 - hypotenuse / TotalLong) * (board2.Height - card0.Height) + card0.Height)
            If MoveCard.Tag = 5 Then MovingCard.Size = New Size((1 - hypotenuse / TotalLong) * (discard_pile.Width - board1.Width) + board1.Width, (1 - hypotenuse / TotalLong) * (discard_pile.Height - board1.Height) + board1.Height)
            If MoveCard.Tag = 3 And Not (MovingCard.Width > MoveTarget.Width - 10 Or MovingCard.Height > MoveTarget.Height - 10) Then
                MovingCard.Width += 10
...
JustSize:
    ...
    Private Sub refresh_handcards()
        ...
        Next
        For x As Integer = 0 To HandCardList(1).Items.Count - 1
    ...
    Private Sub setdamage2_Tick(sender As Object, e As EventArgs) Handles setdamage2.Tick
        ...
        If setdamage2.Tag = 0 AndAlso (damage2.Width > originalDamageSize.Width - 10 Or damage2.Height > originalDamageSize.Height - 10) Then
            setdamage2.Tag = 1
            DamageDelay2 = 50
    ...
    Private Sub setdamage1_Tick(sender As Object, e As EventArgs) Handles setdamage1.Tick
        ...
        If setdamage1.Tag = 0 AndAlso (damage1.Width > originalDamageSize.Width - 10 Or damage1.Height > originalDamageSize.Height - 10) Then
            setdamage1.Tag = 1
            DamageDelay1 = 50
    ...
    Private Sub TurnWeaver_Tick(sender As Object, e As EventArgs) Handles TurnWeaver.Tick
        If setdamage1.Enabled Or setdamage2.Enabled Or setlife1.Enabled Or setlife2.Enabled Or Controls.Contains(MovingCard) Or Enable = False Then Exit Sub
        If TurnWeaver.Tag = 0 Then
            TurnWeaver.Tag = 1
            Dim LoopTime As Boolean = True
        ...
        ElseIf (TurnWeaver.Tag = 7 And ThrowCardOpposite = False) Or (TurnWeaver.Tag = 8 And ThrowCardOpposite) Then
            If board1.Visible Then
                ...
                MoveCard.Tag = 5
                TotalLong = Math.Pow(Math.Pow(discard_pile.Left - board1.Left, 2) + Math.Pow(discard_pile.Top - board1.Top, 2), 0.5)
        ...
        ElseIf (TurnWeaver.Tag = 8 And ThrowCardOpposite = False) Or (TurnWeaver.Tag = 7 And ThrowCardOpposite) Then
            If board2.Visible Then
                ...
                MoveCard.Tag = 5
                TotalLong = Math.Pow(Math.Pow(discard_pile.Left - board2.Left, 2) + Math.Pow(discard_pile.Top - board2.Top, 2), 0.5)
                MoveCard.Enabled = True
            End If
            ...
            For x As Integer = 0 To 1
                For y As Integer = 0 To 2
                    If Statuses(x, y).Visible Then
                        Statuses(x, y).Tag = Statuses(x, y).Tag.split(",")(0) & "," & Statuses(x, y).Tag.split(",")(1) - 1
    ...
    Private Sub MoveAtropos_Tick(sender As Object, e As EventArgs) Handles MoveAtropos.Tick
        ...
        ElseIf Atropos.Tag = 2 Then
            ...
            If tutorial_next.Tag <= 0 Then
                ...
                TimeLabel.Location = New Point((Atropos.Width - TimeLabel.Width) / 2, (Atropos.Height - TimeLabel.Height) / 3)
                TimeLabel.Text = 15
        ...
        ElseIf Atropos.Tag = 9 Then
            ...
            If Atropos.Top >= Height Then
                ...
                My.Computer.Audio.Play(My.Computer.FileSystem.SpecialDirectories.MyDocuments & "\temporary files\victory2.wm")
                Atropos.Tag = 10
    ...
    ''' <returns></returns>
    Private Function UnlockNumber(ByVal password As String) As Single
        Dim exportion As String = ""
    ...
    ''' <returns></returns>
    Private Function GetPicture(ByVal CardName As String) As Image
    ...
    Function GenerateRandom(ByVal Length As Integer, strtemp As String) As String
        ...
        Dim NewRandom As Text.StringBuilder = New Text.StringBuilder(Length)
    ...
    ''' <returns></returns>
    Private Function GetDamageImage(ByVal damage As Single) As Image
        If damage = 0.5 Then
    ...
    Private Sub PlayCard(ByVal TargetCard As PictureBox, targetint As Integer)
        If Enable = False Or TurnWeaver.Enabled Or Controls.Contains(MovingCard) Or Controls.Contains(MovingCardEx) Or TimeLabel.Visible = False Then Exit Sub
        TimeCost.Enabled = False
        TimeLabel.Visible = False
        healbutton.Enabled = False
        OneSuccess = False
        ...
        Dim WordChain As String = ""
        For x As Integer = 0 To HandCardList(0).Items.Count - 1
        ...
        TotalLongEx = Math.Pow(Math.Pow(board2.Left - MovingCardEx.Left, 2) + Math.Pow(board2.Top - MovingCardEx.Top, 2), 0.5)
        MoveCard.Enabled = True
    ...
    End Sub
    Private Function NotACharacter(ByVal text_ As String) As Boolean
        If text_ = "rooster" Or text_ = "hen" Or text_ = "turkey" Or text_ = "witch" Or text_ = "Mr.Duck" Or text_ = "reflect" Then Return False Else Return True
    End Function
    Private Sub CauseDamage(ByVal player1 As Boolean, value As Single)
    ...
    ''' <returns></returns>
    Private Function HasStatus(ByVal player1 As Boolean, status_name As String) As Boolean
        Dim playerInt As Integer
        If player1 Then playerInt = 0 Else playerInt = 1
        For x As Integer = 0 To 2
            If Statuses(playerInt, x).Visible AndAlso Statuses(playerInt, x).Tag.ToString.Split(",")(0) = status_name Then Return True
        Next
        Return False
    End Function
    ''' <summary>
    ...
    Private Sub AddStatus(ByVal player1 As Boolean, status_name As String, turns As Integer)
        Dim playerInt As Integer
        If player1 Then playerInt = 0 Else playerInt = 1
        For x As Integer = 0 To 2
            If Statuses(playerInt, x).Visible = False OrElse Statuses(playerInt, x).Tag.ToString.Split(",")(0) = status_name Then
        ...
        If player1 Then setdamage2.Enabled = True Else setdamage1.Enabled = True
    End Sub
    Private Function GetStatusPopoutImage(ByVal Name_ As String) As Image
        If Name_ = "timed" Then
    ...
    ''' <returns></returns>
    Private Function GetToolTip(ByVal status As PictureBox) As String
        Dim FirstText As String
        If status.Tag.ToString.Split(",")(0) = "timed" Then
            FirstText = "Timed" & vbCrLf & "The player can only have 3 seconds in its turn, if time runs out, his first card will be destroyed."
        ...
        End If
        Return FirstText & vbCrLf & status.Tag.ToString.Split(",")(1) & " turn" & CheckForS(status.Tag.ToString.Split(",")(1)) & " left."
    End Function
    ''' <summary>
    ''' 通过状态名称得到状态图片。
    ''' </summary>
    ''' <param name="status_name">状态名称。</param>
    ''' <returns></returns>
    Private Function GetStatusImage(ByVal status_name As String) As Image
        ...
        End If
    End Function
    Public Function CheckForS(ByVal value As Integer) As String
        If value > 1 Then Return "s" Else Return ""
    End Function
    Private Sub Discard(ByVal player1 As Boolean)
        ...
        Enable = False
        Dim CardIntList As String = ""
        Dim ChosenCardInt As Integer
        For x As Integer = 0 To HandCardList(cardint).Items.Count - 1
